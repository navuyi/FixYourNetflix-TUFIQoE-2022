...
data = {
        ...
        "subject_age": 24,
        "subject_sex": "male",
        "subject_netflix_familiarity": True,
        "subject_selected_content": False,
        "content_continuation": True,
...
# NOTE This test requires empty database
def test_creating_first_experiment(client):
    response = client.get("/experiment/next_id")
    assert response.status_code == 200
    assert response.json["next_id"] == 1

    # Create new experiment with fetched experiment ID
    new_data = data.copy()
    new_data["id"] = 1
    res = client.post("/experiment/", data=json.dumps(new_data), headers=headers)
    assert res.status_code == 201
    assert res.json["msg"] == "Experiment created"

    # Test next ID once again
    response = client.get("/experiment/next_id")
    assert response.status_code == 200
    assert response.json["next_id"] == 2


def test_creating_experiment(client):
    response = client.get("/experiment/next_id")
    assert response.status_code == 200
    ...
    response = client.get("/experiment/next_id")
    assert response.status_code == 200
    assert response.json["next_id"] == next_id+1
